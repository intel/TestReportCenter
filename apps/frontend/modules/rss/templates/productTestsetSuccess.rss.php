...
<?php use_helper("Date") ?>
	<channel>
		<title><?php echo sfConfig::get('app_project_name'); ?> - <?php echo $currentProject["name"]."/".$currentProduct["name"]; ?></title>
		<link><?php echo url_for("@homepage", true); ?></link>
		<description><?php echo sfConfig::get("app_project_group")." ".sfConfig::get('app_project_name')." for ".$currentProject["name"]."/".$currentProduct["name"]; ?></description>
...
Pass rate of executed: <?php echo round($data["pass"] / ($data["pass"] + $data["fail"]) * 100); ?>
				...
				<link><?php echo url_for("testset_session", array("project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $data["te_slug"], "image" => $data["i_slug"], "id" => $data["id"]), true); ?></link>
				<guid><?php echo url_for("testset_session", array("project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $data["te_slug"], "image" => $data["i_slug"], "id" => $data["id"]), true); ?></guid>
			</item>
		<?php endforeach; ?>
	</channel>
</rss>
