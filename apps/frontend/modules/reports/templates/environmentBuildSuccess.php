...
<div id="page">
	<div id="index_page">
		<div id="breadcrumb">
			...
			<li>> <?php echo $currentEnvironment["name"]; ?></li>
		</div> <!-- /breadcrumb -->
			...
			<thead>
				<tr>
					<th class="filtered" scope="col">
						<?php echo $currentProduct["name"]." / ".$currentBuild["build_id"]." / ".$currentEnvironment["name"]; ?>
						<a class="rss" href="<?php echo url_for("rss_build_environment", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"])); ?>" title="RSS feed"></a>
					</th>
				</tr>
			</thead>
				...
				<tr>
					<td class="filtered">
						<div class="chart_actions stack_chart">
							...
							<a id="csv_report_link" href="<?php echo url_for("export_environment_build", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"])); ?>" title="Download as CSV">Download as CSV</a>
						</div>

						<div class="index_month" id="reports_by_month"> <?php
							$previousImage = "";
							$rowCount = 0;
							$globalRowCount = 0;
							$limit = sfConfig::get('app_views_number_of_sessions_in_other_views', 5); ?>
								...
								<?php endif; ?>
											...
											<tr class="<?php echo ($rowCount%2 == 0) ? "even" : "odd"; ?>">
												...
												<td class="report_name">
													<a href="<?php echo url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $data["i_slug"], "id" => $data["id"], "display" => "basic")); ?>" title="See basic report" class="shortcut_link">Basic</a>
													<a href="<?php echo url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $data["i_slug"], "id" => $data["id"], "display" => "detailed")); ?>" title="See detailed report" class="shortcut_link">Detailed</a>
													<a href="<?php echo url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $data["i_slug"], "id" => $data["id"], "display" => "history")); ?>" title="See history report" class="shortcut_link">History</a>
											...
											if($rowCount == $limit && $sessionsForImages[$globalRowCount]["i_name"] == $previousImage)
											{ ?>
												<tr>
													<td colspan="3" class="heading_actions" style="float: none"><a id="<?php echo "link-".$data["i_name"]; ?>" class="hide_show hidden" href="javascript:;" onclick="hideShow(this.id, '<?php echo "reports-".$data["i_name"]; ?>');" title="Hide/show">Show</a></td>
												</tr>
												<tbody id="<?php echo "reports-".$data["i_name"]; ?>" class="hide"> <?php
											} ?>
