...
<?php
	...
	$executedPassRate = ($totalResultsPassed + $totalResultsFailed) > 0 ? round($totalResultsPassed / ($totalResultsPassed + $totalResultsFailed) * 100, 2) : 0;
	...
	if(count($previousSummaryNumbers) >= 1)
	{
		...
		$previousExecutedPassRate = ($previousTotalResultsPassed + $previousTotalResultsFailed > 0) ? round($previousTotalResultsPassed / ($previousTotalResultsPassed + $previousTotalResultsFailed) * 100, 2) : 0;
	}
	else
	{
...
<div id="page">
	<div id="index_page">
		<div id="breadcrumb">
			...
			<li>> <a href="<?php echo url_for("build_reports", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"])); ?>" title="<?php echo $currentProduct["name"]; ?>"><?php echo $currentBuild["build_id"]; ?></a></li>
			<li>> <a href="<?php echo url_for("environment_builds", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"])); ?>" title="<?php echo $currentEnvironment["name"]; ?>"><?php echo $currentEnvironment["name"]; ?></a></li>
			<li>> <a href="<?php echo url_for("image_builds", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $currentImage["name_slug"])); ?>" title="<?php echo $currentImage["name"]; ?>"><?php echo $currentImage["name"]; ?></a></li>
		...
		<div class="page_content">
			...
			<?php if($sf_user->isAuthenticated()): ?>
					...
					<a id="print-button" class="small_btn" href="<?php echo url_for("print_build", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $currentImage["name_slug"], "id" => $currentSession["id"])); ?>" title="Print report">Print</a>
			<?php endif; ?>
				...
				<div class="container">
					<div id="test_results_navi">
						<?php if(isset($previousSession)): ?>
							<a class="go_to_previous" href="<?php echo url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $currentImage["name_slug"], "id" => $previousSession["id"], "display" => $currentDisplay)); ?>" title="Previous report">Previous report</a>
						<?php endif; ?>

						<?php if(isset($nextSession)): ?>
							<a class="go_to_next" href="<?php echo url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $currentImage["name_slug"], "id" => $nextSession["id"], "display" => $currentDisplay)); ?>" title="Next report">Next report</a>
						<?php endif; ?>
					...
					<span class="sort">
						...
						<!-- <a id="detailed_feature" class="see_feature_build_button sort_btn" href="#" title="See build pass%">See build pass%</a> -->
					</span>
					<span style="float: right">
						Switch to:
						<a class="shortcut_link_big<?php if($currentDisplay == "basic") echo "_active"; ?>" href="<?php echo url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $currentImage["name_slug"], "id" => $currentSession["id"], "display" => "basic")); ?>#test_features" title="Basic">Basic</a>
						<a class="shortcut_link_big<?php if($currentDisplay == "detailed") echo "_active"; ?>" href="<?php echo url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $currentImage["name_slug"], "id" => $currentSession["id"], "display" => "detailed")); ?>#test_features" title="Detailed">Detailed</a>
						<a class="shortcut_link_big<?php if($currentDisplay == "history") echo "_active"; ?>" href="<?php echo url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $currentImage["name_slug"], "id" => $currentSession["id"], "display" => "history")); ?>#test_features" title="History">History</a>
					</span>
						...
						<tbody> <?php
							...
							foreach($currentSession["features"] as $feature)
							{
								...
								<tr class="feature_record <?php echo ($line % 2 == 0) ? "odd" : "even"; ?>"> <?php
									...
									$urlFeature = ($currentDisplay != "detailed") ? url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $currentImage["name_slug"], "id" => $currentSession["id"], "display" => "detailed")) : "";
								?>
						...
						<thead class="even">
							<tr>
								...
								<?php for($i=count($previousSessions)-1; $i>=0; $i--): ?>
									<th class="th_history_result"><a href="<?php echo url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $currentImage["name_slug"], "id" => $previousSessions[$i]["id"])); ?>" title="<?php echo "(".$previousSessions[$i]["id"].") ".$previousSessions[$i]["name"]; ?>"><?php echo format_datetime($previousSessions[$i]["created_at"], "y-MM-dd HH:mm")." Build ID: ".$currentSession["build_id"]; ?></a></th>
								<?php endfor; ?>
						...
						<tbody> <?php
							...
							foreach($currentSession["features"] as $feature)
							{
								...
								<tr class="feature_record <?php echo ($line % 2 == 0) ? "odd" : "even"; ?>"> <?php
									...
									$urlFeature = ($currentDisplay != "history") ? url_for("build_session", array("build" => $currentBuild["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $currentEnvironment["name_slug"], "image" => $currentImage["name_slug"], "id" => $currentSession["id"], "display" => "history")) : "";
								?>
