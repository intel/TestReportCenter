...
<div id="page">
	<div id="index_page">
		<div id="breadcrumb">
			<li><a href="<?php echo url_for("project_builds", array("project" => $currentProject["name_slug"], "filter" => "recent")); ?>" title="Home">Home</a></li>
			<li>> <?php echo $currentProduct["name"]; ?></li>
		</div> <!-- /breadcrumb -->
			...
			<thead>
				<tr>
					<th class="filtered" scope="col">
						...
						<a class="rss" href="<?php echo url_for("rss_build_product", array("project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"])); ?>" title="RSS feed"></a>
					</th>
				</tr>
			</thead>
				...
				<tr>
					<td class="filtered">
						<div class="chart_actions stack_chart">
							...
							<a id="csv_report_link" href="<?php echo url_for("export_product_build", array("project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"])); ?>" title="Download as CSV">Download as CSV</a>
						</div>
						...
						<div class="index_month" id="reports_by_month"> <?php
							...
							<?php foreach($sessionsForImages as $data): ?>
								<?php if($previousBuild != $data["build_id"]): ?>
									<?php $rowCount = 0; ?>
									<table class="month">
										<thead>
											<tr>
												<td class="index_month_title" colspan="3">
													<strong class="name"><?php echo $data["build_id"]; ?></strong>
												</td>
											</tr>
										</thead>
								...
								<?php endif; ?>
											...
											<tr class="<?php echo ($rowCount%2 == 0) ? "even" : "odd"; ?>">
												...
												<td class="report_name">
													<a href="<?php echo url_for("build_session", array("build" => $data["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $data["te_slug"], "image" => $data["i_slug"], "id" => $data["id"], "display" => "basic")); ?>" title="See basic report" class="shortcut_link">Basic</a>
													<a href="<?php echo url_for("build_session", array("build" => $data["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $data["te_slug"], "image" => $data["i_slug"], "id" => $data["id"], "display" => "detailed")); ?>" title="See detailed report" class="shortcut_link">Detailed</a>
													<a href="<?php echo url_for("build_session", array("build" => $data["build_slug"], "project" => $currentProject["name_slug"], "product" => $currentProduct["name_slug"], "environment" => $data["te_slug"], "image" => $data["i_slug"], "id" => $data["id"], "display" => "history")); ?>" title="See history report" class="shortcut_link">History</a>
											...
											if($rowCount == $limit && $sessionsForImages[$globalRowCount]["build_id"] == $previousBuild)
											{ ?>
												<tr>
								...
								<?php $previousBuild = $data["build_id"]; ?>
							<?php endforeach; ?>
